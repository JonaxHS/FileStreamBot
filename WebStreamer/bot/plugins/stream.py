...
async def private_receive_handler(c: Client, m: Message):
    if not await db.is_user_exist(m.from_user.id):
        ...
        await c.send_message(
            Var.BIN_CHANNEL,
            f"Nuevo usuario registrado : \n\nNᴀᴍᴇ : [{m.from_user.first_name}](tg://user?id={m.from_user.id}) Utiliza el BOT !!"
        )
    if Var.UPDATES_CHANNEL != "None":
        try:
            ...
            if user.status == "kicked":
                await c.send_message(
                    chat_id=m.chat.id,
                    text="__Lo siento señor, Has sido votado.__\n\n  **Contactame para ayudarte @mistreamgo**",
                    parse_mode="markdown",
            ...
            await c.send_message(
                chat_id=m.chat.id,
                text="""<i>Unete al canal de actualizaciones para usarme 🔐</i>""",
                reply_markup=InlineKeyboardMarkup(
                    [[ InlineKeyboardButton("Unirme 🔓", url=f"https://t.me/{Var.UPDATES_CHANNEL}") ]]
                ),
                parse_mode="HTML"
            )
            return
            ...
            await c.send_message(
                chat_id=m.chat.id,
                text="**Sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ Wʀᴏɴɢ. Cᴏɴᴛᴀᴄᴛ ᴍʏ ʙᴏss** @Mistreamgo",
                parse_mode="markdown",
                disable_web_page_preview=True)
            return
    try:
...
<i>© @Mistreamgo </i>"""
        ...
        await m.reply_text(
            ...
            parse_mode="HTML", 
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Dᴏᴡɴʟᴏᴀᴅ ɴᴏᴡ 📥", url=f"http://mistream.rf.gd/video.php?u={stream_link}&t={file_name}"),
                                               InlineKeyboardButton("Dᴏᴡɴʟᴏᴀᴅ ɴᴏᴡ 📥", url=f"intent:http://devimages.apple.com/iphone/samples/bipbop/bipbopall.m3u8#Intent;package=com.mxtech.videoplayer.ad;end")]]),
            quote=True
        )
